    ...
    const startConnSttStream = () => {
      ...
      connSttStream.events.on(
        ...
        async ({ text, isFinal }: { text: string; isFinal: boolean }) => {
          ...
          // Run risk analysis
          console.log(
            `🔎 [STT-stream] final transcript roomId=${roomId}, text="${text.substring(0, 80)}${text.length > 80 ? "..." : ""}"`,
          );
          try {
            ...
            if (room) {
              console.log(
                `🏢 [STT-stream] room found, companyType=${room.companyType} → calling processTranscriptAnalysis`,
              );
